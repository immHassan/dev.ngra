...
class Product extends Admin_Controller {
    function __construct() {
        ...
        parent::__construct();
        $this->image_fields = array('product_image'); 
        $this->multi_image_fields = array();; 
        $this->orderable = true;  
        $this->show_fields = array('product_name','category_id','product_description','product_featured','product_price','product_sale_price','product_image');
    }  
    ...
    public function form($form_choice = "",$id="")
    {
            ...
            $category_query['table'] = 'category';
            $category_array = $this->general->get($category_query);
            foreach($category_array as $key){
                $content['category_array'][$key['category_name']] = $key['category_id'];
            }


            $currency_query['select'] = 'currency_id,currency_name';
            $currency_query['table'] = 'currency';            
            $currency_array = $this->general->get($currency_query);
            foreach($currency_array as $key){
                $content['currency_array'][$key['currency_name']] = $key['currency_id'];
            }
        ...
        if($_POST){
            $content = array(
				...
				'product_sale_price' => $this->input->post('product_sale_price',TRUE),
				'product_currency' => $this->input->post('product_currency',TRUE),
